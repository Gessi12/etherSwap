...
contract('Exchange',([deployer, feeAccount, user1, user2]) => {
    ...
    describe('depositing Ether',async ()=>{
        ...
        it('tracks the Ether deposit',async ()=>{
            const balance = await exchange.tokens(ETHERADDRESS,user1)
        ...
        it('emits a Deposit event',async ()=>{
            ...
            event.token.toString().should.equal(ETHERADDRESS,"token address is correct")
    ...
    describe('depositing tokens',async ()=>{
        ...
        describe('failure',()=>{
            it('reject Ether deposit',async() =>{
                //TODO: fill me in...
                await exchange.depositToken(ETHERADDRESS, tokens(10), { from: user1}).should.be.rejectedWith(EVM_REVERT)
            })

            it('fails when no tokens are approved', async()=>{
                //Do not approve any tokens before depositing
    ...
    describe('withdrawing Ether',async ()=>{
        ...
        describe('failure',async ()=>{
            ...
            it('fails for insufficient balances', async()=>{
                //Attempt to withdraw tokens without depositing ant firse
    ...
    describe('checking balance',async()=>{
        ...
        it('return user balance',async ()=>{
            const result = await exchange.balanceOf(ETHERADDRESS,user1)
    ...
    describe('making orders',async ()=>{
        let result
        beforeEach(async ()=>{
            result = await exchange.makeOrder(token.address,tokens(1),ETHERADDRESS,ether(1),{ from: user1 })
        })
        ...
        it('tracks the newly created order',async ()=>{
            ...
            order.tokenGive.should.equal(ETHERADDRESS,'tokenGive is correct')
        ...
        it('emits an "Order event',async ()=>{
            ...
            event.tokenGive.should.equal(ETHERADDRESS,'tokenGive is correct')
    ...
    describe('order actions', async () =>{
        ...
        beforeEach(async () =>{
            ...
            // user1 makes an order to buy tokens with Ether
            await exchange.makeOrder(token.address, tokens(1), ETHERADDRESS, ether(1), { from: user1})
        })
        ...
        describe('filling orders', async ()=>{
            ...
            describe('success', async()=>{
                ...
                it('executes the trade & change fees', async ()=>{
                    let balance
                    balance = await exchange.balanceOf(token.address, user1)
                    balance.toString().should.equal(tokens(1).toString(), 'user1 received tokens')
                    balance = await exchange.balanceOf(ETHERADDRESS, user2)
                    balance.toString().should.equal(ether(1).toString(), 'user2 received Ether')
                    balance = await exchange.balanceOf(ETHERADDRESS, user1)
        ...
        describe('cancelling orders', async ()=>{
            ...
            describe('failure', async ()=>{
                ...
                it('rejects unauthorized cancelations',async ()=>{
                    // Try to cancel the order from anther user
